 ...
 *
 * Then, we go back to the start of the row, giving n + 1 candies to the first person, n + 2 candies to the second person, and
...
var distributeCandies = function (candies, num_people) {
  ...
  while (candies > 0) {
    ...
    let candyLeft = loops * num_people + (i + 1);
    if (candies - candyLeft < 0) {
      arr[i] += candies;
    } else {
      arr[i] += candyLeft;
    }
